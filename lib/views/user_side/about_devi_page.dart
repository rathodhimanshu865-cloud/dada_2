...
class AboutDeviPage extends StatefulWidget {
  const AboutDeviPage({super.key});

  @override
  State<AboutDeviPage> createState() => _AboutDeviPageState();
}

class _AboutDeviPageState extends State<AboutDeviPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    const primaryTeal = Color(0xFF5A1212); // Shifting to a deeper reddish-teal/maroon for Devi theme
    const backgroundBeige = Color(0xFFFDF8F5);
    const accentBrown = Color(0xFFD48166); // Warm red-gold accent
    ...
    return UserPageLayout(
      ...
      child: VisibilityDetector(
        key: const Key('about-devi-visibility'),
          ...
          children: [
            ...
            Padding(
              ...
              child: isMobile 
                ? Column(
                    children: [
                      FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1200), child: _buildHighlightCard(data.localizedHighlight1Title(lang), Icons.auto_awesome_outlined, data.localizedHighlight1Desc(lang), accentBrown)),
                      const SizedBox(height: 30),
                      FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1400), child: _buildHighlightCard(data.localizedHighlight2Title(lang), Icons.brightness_high_outlined, data.localizedHighlight2Desc(lang), accentBrown)),
                      const SizedBox(height: 30),
                      FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1600), child: _buildHighlightCard(data.localizedHighlight3Title(lang), Icons.auto_fix_high_rounded, data.localizedHighlight3Desc(lang), accentBrown)),
                    ],
                  )
                : Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Expanded(child: FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1200), child: _buildHighlightCard(data.localizedHighlight1Title(lang), Icons.auto_awesome_outlined, data.localizedHighlight1Desc(lang), accentBrown))),
                      const SizedBox(width: 40),
                      Expanded(child: FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1400), child: _buildHighlightCard(data.localizedHighlight2Title(lang), Icons.brightness_high_outlined, data.localizedHighlight2Desc(lang), accentBrown))),
                      const SizedBox(width: 40),
                      Expanded(child: FadeInUp(animate: _isVisible, delay: const Duration(milliseconds: 1600), child: _buildHighlightCard(data.localizedHighlight3Title(lang), Icons.auto_fix_high_rounded, data.localizedHighlight3Desc(lang), accentBrown))),
                    ],
                  ),
            ),
            ...
            const SizedBox(height: 120),
            _RelatedKathas(primaryTeal: primaryTeal, accentGold: accentBrown, isMobile: isMobile, currentPath: '/about_devi_katha'),
