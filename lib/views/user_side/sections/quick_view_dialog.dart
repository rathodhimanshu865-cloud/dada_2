...
class _QuickViewDialogState extends State<QuickViewDialog> {
  ...
  Widget _buildInfoSection(BuildContext context, Color teal, Color gold, CartController cart, AuthController auth) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: teal.withOpacity(0.05), borderRadius: BorderRadius.circular(4)), child: Text(widget.product.categoryId.toUpperCase(), style: AppTypography.bodyStyle(context, color: teal, fontWeight: FontWeight.bold, fontSize: 10, letterSpacing: 1.5))),
        ...
        const SizedBox(height: 20),
        Row(children: [Row(children: List.generate(5, (i) => Icon(i < 4 ? Icons.star : Icons.star_half, size: 18, color: gold))), const SizedBox(width: 10), Text('4.95', style: AppTypography.bodyStyle(context, fontWeight: FontWeight.w900, fontSize: 14)), const SizedBox(width: 6), Text('(218 devotee reviews)', style: AppTypography.bodyStyle(context, color: Colors.grey.shade500, fontSize: 13))]),
        const SizedBox(height: 25),
        Row(children: [if (widget.product.discountPrice != null) ...[Text('₹${widget.product.discountPrice!.toInt()}', style: AppTypography.headingStyle(context, fontSize: 32, color: teal, fontWeight: FontWeight.w900)), const SizedBox(width: 12), Text('₹${widget.product.price.toInt()}', style: TextStyle(decoration: TextDecoration.lineThrough, color: Colors.grey.shade400, fontSize: 18)), const SizedBox(width: 12), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(4), border: Border.all(color: Colors.green.shade100)), child: Text('${((widget.product.price - widget.product.discountPrice!) / widget.product.price * 100).toInt()}% OFF', style: const TextStyle(color: Colors.green, fontWeight: FontWeight.w900, fontSize: 11)))] else Text('₹${widget.product.price.toInt()}', style: AppTypography.headingStyle(context, fontSize: 32, color: teal, fontWeight: FontWeight.w900))]),
        const SizedBox(height: 20),
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: (widget.product.isActive && widget.product.stock > 0) ? const Color(0xFFECFDF5) : const Color(0xFFFEF2F2), borderRadius: BorderRadius.circular(6), border: Border.all(color: (widget.product.isActive && widget.product.stock > 0) ? const Color(0xFFD1FAE5) : const Color(0xFFFEE2E2))), child: Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 8, height: 8, decoration: BoxDecoration(color: (widget.product.isActive && widget.product.stock > 0) ? const Color(0xFF10B981) : Colors.red, shape: BoxShape.circle)), const SizedBox(width: 8), Text((widget.product.isActive && widget.product.stock > 0) ? 'In Stock (${widget.product.stock} available) • Fast Dispatch' : 'Out of Stock', style: AppTypography.bodyStyle(context, color: (widget.product.isActive && widget.product.stock > 0) ? const Color(0xFF065F46) : Colors.red.shade900, fontSize: 12, fontWeight: FontWeight.bold))])),
        const SizedBox(height: 30),
